...
import numpy as np
# FIX: added validation_data parameter to avoid data leak from validation_split
# FIX: added batch_size parameter for hyperparameter search
def train_model(model, X_train, y_train, model_name, validation_data, epochs=50, batch_size=32, early_stopping=True, patience=50, verbose=1):
    ...
    history = model.fit(
        X_train, y_train,
        # FIX: validation_data is now required (no fallback to validation_split)
